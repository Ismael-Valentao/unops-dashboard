...
const Balances = {
  ...
  async summary(opts = {}) {
    ...
    // FALTA per-row = SUM(MAX(0, planned - committed)) — operacionalmente
    // a quantidade que ainda tem de ser DIRECTAMENTE entregue. Excesso a um
    // benef (over-delivery) não reduz o défice de outro porque cada benef
    // recebe só o que é entregue a ele. Bate com o somatório da coluna
    // "Falta" da tabela. Diferença vs Plano−Entregue é exactamente o
    // excesso (mostrado à parte como "Excesso").
...
const Services = {
  ...
  async dashboard() {
    ...
    // Por SKU: falta = SUM(MAX(0, planned - committed)) per-benef. Excesso
    // a um benef não compensa défice a outro — cada benef recebe só o que
    // é entregue a ele directamente.
    ...
    //    Falta per-row, convertida para kg-equivalente.
