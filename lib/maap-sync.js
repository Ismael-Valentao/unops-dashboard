      ...
      await conn.query(
        `INSERT INTO delivery_balances
         ...
         ON DUPLICATE KEY UPDATE
           ...
           planned_qty = GREATEST(0, VALUES(planned_original) - realocado_recebido)`,
        [c.ext_id, c.sku, c.label, c.unit,
         benef.province, benef.district, benef.name || c.ext_id,
         c.new_planned_original, c.new_planned_qty]
      );
      stats.nBalCreated++;
    }

    // 2d. ACTUALIZAR saldos existentes (planned_original e planned_qty)
    onLog(`[apply] A actualizar ${plan.balances_to_update.length} saldos...`);
    for (const u of plan.balances_to_update) {
      // FÓRMULA: new_planned_qty = new_planned_original - realocado_recebido
      // Mas Math.max(0, ...) caso realocado > novo plano (raro)
      await conn.query(
        `UPDATE delivery_balances
         SET planned_original = ?,
             planned_qty = GREATEST(0, ? - realocado_recebido)
         WHERE extensionist_id = ? AND sku = ?`,
        [u.new_planned_original, u.new_planned_original, u.ext_id, u.sku]
      );
      stats.nBalUpdated++;
    }
