 ...
 *
 * Chave de agrupamento: ADSE (ParentServiceCode) — cada ADSE é uma
 * viagem/carga única do camião. Múltiplos ADSNs (filhos) partilham
 * o mesmo ADSE quando saem do mesmo carregamento. Fallback para
 * matricula+data+batedor se item não tem ADSE (caso raro).
 */
function groupByViagem(entregas, supervisorMap = new Map()) {
  ...
  for (const e of entregas) {
    ...
    // Chave principal: ADSE (canónica da viagem). Fallback: matricula+data+batedor.
    const adse = String(e.adse || "").trim();
    const key = adse
      ? "adse:" + adse
      : "fallback:" + [
    ...
    // Defensivo: se a primeira row do ADSE não tinha matricula/transportador
    // mas uma segunda tem, preenche (algumas ADSNs podem ter dados em falta)
    if (!v.matricula && e.truck_plate) {
      v.matricula = e.truck_plate;
      v.matricula_norm = normPlate(e.truck_plate);
    }
    if (!v.motorista && e.driver) v.motorista = e.driver;
    if (!v.transportador && e.transporter_co) v.transportador = e.transporter_co;
    if (!v.fornecedor && e.supplier) v.fornecedor = e.supplier;
    if (!v.origem && e.supplier_origin) v.origem = e.supplier_origin;
  }
  ...
  for (const v of byKey.values()) {
    ...
    // Breakdown por produto: a viagem pode misturar SKUs (ex: Milho + Feijão).
    // Agregamos por nome amigável do produto e devolvemos array ordenado por kg desc.
    const productsMap = new Map();
    for (const it of v.items) {
      const p = it.product || "—";
      const cur = productsMap.get(p) || { product: p, kg: 0, volumes: 0, count: 0 };
      cur.kg      += Number(it.weight_kg) || 0;
      cur.volumes += Number(it.volumes)   || 0;
      cur.count   += 1;
      productsMap.set(p, cur);
    }
    v.products = [...productsMap.values()]
      .map((p) => ({
        product: p.product,
        kg:      Math.round(p.kg),
        ton:     Number((p.kg / 1000).toFixed(3)),
        volumes: p.volumes,
        count:   p.count,
      }))
      .sort((a, b) => b.kg - a.kg);
    v.n_products = v.products.length;
    // Agregação de match de quantidade UNOPS:
    //   - n_qty_mismatch: ADSNs com sheet match mas qty diverge (qty_match=false)
    //   - n_qty_match:    ADSNs com sheet match E qty bate (qty_match=true)
    //   - sum_qty_diff_kg: soma de (sheet − api) para os ADSNs com sheet
    v.n_qty_mismatch  = v.items.filter((i) => i.qty_match === false).length;
    v.n_qty_match     = v.items.filter((i) => i.qty_match === true).length;
    v.sum_qty_diff_kg = v.items
      .filter((i) => i.qty_diff_kg != null)
      .reduce((s, i) => s + i.qty_diff_kg, 0);
    v.sum_qty_diff_kg = Number(v.sum_qty_diff_kg.toFixed(2));
    // Agregação de match de NOME — submissões UNOPS com beneficiário diferente
    v.n_name_mismatch = v.items.filter((i) => i.name_match === false).length;
    v.n_name_match    = v.items.filter((i) => i.name_match === true).length;
