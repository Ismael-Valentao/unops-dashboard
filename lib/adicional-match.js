 ...
 * Variantes aceites e canonicalizadas:
 *   "GTU98/2023/6543"   → "GTU98/202306543"   (slash extra + padding 5d)
 *   "GTU98/2023/06543"  → "GTU98/202306543"   (idem)
 *   "GTU98/2023/26543"  → "GTU98/202326543"   (já 5 dígitos)
 *   "GTU98/202306543"   → "GTU98/202306543"   (formato canónico — sem mudança)
 *   "  gtu98/202306159 "→ "GTU98/202306159"   (trim + upper)
 *
 * Regra: após o ano (YYYY) o número de sequência é zero-padded a 5 dígitos.
 */
function normGtu(s) {
  let g = String(s || "").trim().toUpperCase().replace(/\s+/g, "");
  // Forma com slash extra: PREFIX/YYYY/NNNN → PREFIX/YYYYNNNNN (pad 5)
  g = g.replace(
    /^([A-Z]+\d*)\/(\d{4})\/(\d+)$/,
    (_m, prefix, year, seq) => prefix + "/" + year + seq.padStart(5, "0")
  );
  // Defensivo: se o formato é PREFIX/YYYYNNN (sem slash extra) mas só 3-4
  // dígitos depois do ano, pad também. Ex: GTU98/202345 → GTU98/202300045.
  // Mas: cuidado — não queremos mexer em IDs que já têm 5+ dígitos.
  g = g.replace(
    /^([A-Z]+\d*)\/(\d{4})(\d{1,4})$/,
    (_m, prefix, year, seq) => prefix + "/" + year + seq.padStart(5, "0")
  );
