...
// Pode ser sobrescrito via env MAAP_DIR.
const DEFAULT_DIR = process.env.MAAP_DIR
  || "C:/Users/Ismael Chiziane/Documents/AQI/Planeamentos/Actualizações";
...
function parseMAAPFile(filepath, provLabel) {
  ...
  for (const r of rows) {
    ...
    if (!id) {
      ...
      pendingIdx++;
      id = `MAAP-PEND-${provLabel.toUpperCase()}-${String(pendingIdx).padStart(3, "0")}`;
    }
...
module.exports = { loadMAAP, applyKitRecipe, KIT_RECIPE, FILES, DEFAULT_DIR };
