 ...
 */
function buildEntregas({ apiRows = [], sheetRows = [], batedoresMap = new Map(), beneficiariesMap = new Map() }) {
  ...
  const entregas = apiRows.map((s) => {
    ...
    return {
      ...
      weight_ton:      Number(((Number(s.Weight) || 0) / 1000).toFixed(2)),
      ...
      sheet_qty_kg: matched[0]?.delivered_qty != null ? Number(matched[0].delivered_qty) : null,
    };
  });
