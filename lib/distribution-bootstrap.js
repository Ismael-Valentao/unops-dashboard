...
// A chave é a variante em title-case (output da normalização base), não o raw
// do Excel. Ampliar à medida que aparecerem novas variantes.
const DISTRICT_ALIASES = {
  "Chongoene": "Chonguene",
};

// Normaliza nome de distrito (consistente com app.js).
// Faz title-case e depois colapsa variantes ortográficas conhecidas.
  ...
  for (const r of rows) {
    ...
    if (!benefMap.has(extId)) {
      benefMap.set(extId, {
        ...
        localidade: String(r["Localidade"] || "").trim() || null,
    ...
    // Tipo de Feijão (Vulgar / Nhemba / Nhemba e Vulgar). Só vem em rows
    // de Feijão. Quando o mesmo benef tem múltiplas rows com tipos
    // diferentes, mostra como "Nhemba e Vulgar".
    const tipo = String(r["Tipo de Feijão"] || r["Tipo de Feijao"] || "").trim();
    if (tipo) {
      if (!bal.bean_type) {
        bal.bean_type = tipo;
      } else if (bal.bean_type !== tipo) {
        // Linha já tinha um tipo, agora aparece outro → Mistura.
        // "Nhemba e Vulgar" é o label oficial.
        bal.bean_type = "Nhemba e Vulgar";
      }
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < benefList.length; i += 200) {
      ...
      chunk.forEach((b) => params.push(
        ...
        b.posto, b.localidade, b.contact, b.supervisor_name, b.supervisor_phone, ts
      ));
      await conn.query(
        `INSERT INTO beneficiaries
         (extensionist_id, nuit, name, province, district, posto, localidade,
          contact, supervisor_name, supervisor_phone, imported_at)
         VALUES ${placeholders}
         ON DUPLICATE KEY UPDATE
           nuit=VALUES(nuit), name=VALUES(name), province=VALUES(province),
           district=VALUES(district), posto=VALUES(posto),
      ...
      await conn.query(
        `INSERT INTO delivery_balances
         (extensionist_id, sku, product_name, unit, province, district,
          ...
          bean_type, committed_qty, delivered_qty)
         VALUES ${placeholders}
         ON DUPLICATE KEY UPDATE
...
async function importServices(filePath, opts = {}) {
  ...
  for (const r of rows) {
    ...
    // Sacos herméticos (SUSSACO / unit='un') medem-se em UNIDADES, não em kg.
    // No Excel original, alguns rows antigos meteram o count em Peso e outros
    // rows novos meteram o peso em kg em Peso e o count em Volumes. Volumes
    // é sempre ground truth: usar essa coluna para qualquer SKU 'un'.
    const skuMetaForRow = planSkuMeta(sku);
    const isUnit = skuMetaForRow.unit === "un";
    const pesoCol = Number(r["Peso"]) || 0;
    const volumesCol = Number(r["Volumes"]) || 0;
    const peso = isUnit ? volumesCol : pesoCol;
    const estado = String(r.Estado || "").trim().toUpperCase();
    // GTUS98/... é typo recorrente do operador → normaliza para GTS98/... antes
    // de qualquer filtro/match. Após esta linha, trabalho nunca terá "GTUS".
    const trabalho = String(r["Trabalho"] || "").trim().replace(/^GTUS/i, "GTS");
    ...
    groups.get(key).items.push({
      ...
      product_name: skuMetaForRow.name,
      unit: skuMetaForRow.unit,
      province: String(r.Provincia || "").trim(),
      district: normalizeDistrict(r.Distrito),
    });
  }
