...
async function importServices(filePath, opts = {}) {
  ...
  for (const r of rows) {
    ...
    groups.get(key).items.push({
      ...
      product_name: skuMeta(sku).name,
      unit: skuMeta(sku).unit,
      province: String(r.Provincia || "").trim(),
      district: normalizeDistrict(r.Distrito),
    });
  }
  ...
  try {
    for (const g of groups.values()) {
      ...
      try {
        ...
        const totalKg = newItems.reduce((s, it) => s + it.qty, 0);
...
module.exports = { importPlanning, importServices, normalizePlate, skuMeta, mapState };
