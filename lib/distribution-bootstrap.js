...
async function importPlanning(filePath, opts = {}) {
  ...
  const sheet = wb.Sheets["Planeamento Adicional"];
  if (!sheet) throw new Error("Sheet 'Planeamento Adicional' não encontrada em " + filePath);
  ...
  for (const r of rows) {
    ...
    const qty = Number(r["NOVA QUANTIDADE A ENTREGAR"]) || 0;
    ...
    balanceMap.get(k).planned_qty += qty;
  }
  ...
  try {
    ...
    for (let i = 0; i < balList.length; i += 200) {
      ...
      chunk.forEach((b) => params.push(
        ...
        b.province, b.district, b.beneficiary_name, b.planned_qty
      ));
      await conn.query(
...
async function importServices(filePath, opts = {}) {
  ...
  for (const r of rows) {
    ...
    const extId = nuitToExt.get(nuit);
    if (!extId) { skipped_no_benef++; continue; }
      ...
      try {
        ...
        const [svcRes] = await conn2.query(
          ...
          [
            ...
            g.plate1, g.plate2, g.supplier, g.state, totalKg, "imported", ts,
            g.state === "in_transit" || g.state === "delivered" ? ts : null,
            g.state === "delivered" ? ts : null,
            fileName,
          ]
        );
