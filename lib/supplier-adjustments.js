...
const ADJUSTMENTS = [
  ...
  // Pré-credita kg que vamos receber via reassignment quando rows
  // específicas chegarem. Vai descontando à medida que as reais entram
  // (sentinel REASSIGN_MOZSEEDS_CHIGUBO).
  {
    type: "placeholder_minus_matched",
    to_meta_key: "MOZSEEDS",
    product: "Feijão",
    placeholder_qty: 25740,
    sentinel: "REASSIGN_MOZSEEDS_CHIGUBO",
    note: "Placeholder antecipa Feijão ADICIONAL CHIMOIO → Chigubo. " +
          "Desconta automaticamente conforme reais entrarem.",
  },
];
...
function applyAdjustments(suppliers, rows) {
  // rows é OPCIONAL — usado apenas pelo type "placeholder_minus_matched"
  // que precisa de contar matches reais já reassignados. Outros types
  // continuam a funcionar sem rows (backward-compat).
  const FORN_STATUSES = new Set(["TRANSITO", "FINALIZADO"]);
  ...
  for (const adj of ADJUSTMENTS) {
    ...
    if (adj.type === "fixed") {
      ...
      tgt = suppliers.find((s) => s.meta_match_key === adj.to_meta_key);
      qty = adj.qty;
    } else if (adj.type === "placeholder_minus_matched") {
      // Placeholder antecipa kg que esperamos receber por reassignment de
      // rows que ainda não chegaram. À medida que rows reais com o sentinel
      // entram, a placeholder decresce automaticamente: max(0, placeholder − matched).
      // Quando matched ≥ placeholder, é skipped (placeholder consumido).
      src = null;
      tgt = suppliers.find((s) => s.meta_match_key === adj.to_meta_key);
      if (!tgt) { skipped.push({ adj, reason: "tgt not found" }); continue; }
      let matchedKg = 0, matchedCount = 0;
      const sentinel = String(adj.sentinel || "");
      if (sentinel && Array.isArray(rows)) {
        for (const row of rows) {
          if (String(row.ShipAcountNumber || "") !== sentinel) continue;
          if (!FORN_STATUSES.has(String(row.StatusName || "").toUpperCase())) continue;
          matchedKg += Number(row.Weight) || 0;
          matchedCount++;
        }
      }
      qty = Math.max(0, Number(adj.placeholder_qty) - matchedKg);
      if (qty === 0) {
        skipped.push({ adj, reason: "placeholder totalmente consumido por " + matchedCount + " rows reais (" + Math.round(matchedKg) + " kg)" });
        continue;
      }
