...
let memCache = null; // { adsnToCam, builtAt, fileSize, count, sourceUrl }
...
/**
 * Download via curl. Retorna path do ficheiro e tamanho.
 */
function downloadXlsx(url) {
  return new Promise((resolve, reject) => {
    ...
    execFile("curl", args, { maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
      if (err) return reject(new Error("curl falhou: " + err.message + (stderr ? " | " + stderr.slice(0, 200) : "")));
      const [code, ctype, size] = String(stdout).split("|");
      if (code !== "200") return reject(new Error("OneDrive HTTP " + code + " (content-type=" + ctype + ")"));
      ...
      if (sz < 10000) return reject(new Error("Ficheiro descarregado demasiado pequeno: " + sz + " bytes"));
      resolve({ path: CACHE_XLSX, size: sz });
    });
  });
}
...
async function getMapa(opts = {}) {
  ...
  inflightFetch = (async () => {
    try {
      const { path: filePath, size } = await downloadXlsx(url);
