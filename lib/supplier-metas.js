...
const METAS = {
  ...
  // ── Novos fornecedores ─────────────────────────────────────────
  // GLOBAL AGRIBUSINESS, EI - TENDERS é o nome completo na API
  "GLOBAL AGRIBUSINESS": {
    "Arroz": 10000,
  },
  ETG: {
  ...
  // SANA (organizações MH-Tenders) — todos os sacos herméticos são reassignados
  // para "SANA (organizações MH-Tenders)" pelo supplier-reassignments. Match
  // contra a key "SANA" via substring (normName preserva parens, então
  // qualquer nome que contenha "SANA" dá match).
  // Meta em UNIDADES (un), não kg.
  "SANA": {
...
function getMetasFor(canonicalName) {
  ...
  // Phase 2: supplier contains meta key. Ordena por LONGER key primeiro
  // para evitar match prematuro (ex: "GLOBAL AGRIBUSINESS EI" deve bater
  // com a key "GLOBAL AGRIBUSINESS" antes de uma hipotética "AGRIBUSINESS").
