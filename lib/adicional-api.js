...
function inAllowedSeries(barcode) {
  ...
  // Verifica o prefixo na forma CANÓNICA — caso contrário GTUs escritas como
  // "GTU/98/202310231" são rejeitadas porque NÃO começam por "GTU98/".
  // Importa normGtu localmente para evitar dependência circular no top.
  let normalized;
  try {
    const { normGtu } = require("./adicional-match");
    normalized = normGtu(barcode);
  } catch (_) {
    normalized = String(barcode || "").trim().toUpperCase().replace(/\s+/g, "");
  }
  if (!normalized) return false;
  return ALLOWED_SERIES.some((p) => normalized === p || normalized.startsWith(p + "/"));
}
