...
const RULES = [
  ...
  {
    // Feijão ADICIONAL CHIMOIO destinado a Chigubo também é MOZSEEDS.
    // Usa sentinel próprio (REASSIGN_MOZSEEDS_CHIGUBO) para distinguir
    // do Massangena — a placeholder Chigubo (25740 kg) decresce
    // automaticamente à medida que estas rows entram.
    name: "Feijão ex-ADICIONAL CHIMOIO destinado a Chigubo → MOZSEEDS",
    match: (r) => isFeijao(r.SKU)
              && _origStartsWithAdicional(r.ShipFromName)
              && inChimoio(r.ShipFromAddress)
              && /chigubo/i.test(String(r.ReceiverPostalPlace || "")),
    apply: { ShipFromName: "MOZSEEDS", ShipAcountNumber: "REASSIGN_MOZSEEDS_CHIGUBO" },
  },
  {
    // Arroz que saiu do armazém ADICIONAL NICOADALA (Zambézia) é da MOZSEEDS.
    // Destinos típicos: Chinde, Mopeia, Morrumbala, Nicoadala (todos Zambézia).
    name: "Arroz ex-ADICIONAL NICOADALA → MOZSEEDS",
    match: (r) => isArroz(r.SKU)
              && _origStartsWithAdicional(r.ShipFromName)
              && /nicoadala/i.test(String(r.ShipFromAddress || "")),
    apply: { ShipFromName: "MOZSEEDS", ShipAcountNumber: "REASSIGN_MOZSEEDS" },
  },
  {
  ...
  {
    // Feijão ADICIONAL CHIMOIO destinado a Guija → MOZSEEDS.
    // Usa sentinel REASSIGN_MOZSEEDS_CHIGUBO para que a placeholder
    // "antecipação Chigubo" (25 740 kg em supplier-adjustments) decresça
    // automaticamente — as guias foram lançadas como Guija mas
    // correspondem ao volume que se antecipou para Chigubo.
    name: "Feijão ex-ADICIONAL CHIMOIO destinado a Guija → MOZSEEDS",
    match: (r) => isFeijao(r.SKU)
              && _origStartsWithAdicional(r.ShipFromName)
              && inChimoio(r.ShipFromAddress)
              && /guija/i.test(String(r.ReceiverPostalPlace || "")),
    apply: { ShipFromName: "MOZSEEDS", ShipAcountNumber: "REASSIGN_MOZSEEDS_CHIGUBO" },
  },
  {
    // Feijão ADICIONAL CHIMOIO destinado a NAMAACHA → SEEDCO.
    name: "Feijão ex-ADICIONAL CHIMOIO destinado a NAMAACHA → SEEDCO",
    match: (r) => isFeijao(r.SKU)
              && _origStartsWithAdicional(r.ShipFromName)
              && inChimoio(r.ShipFromAddress)
              && /namaacha/i.test(String(r.ReceiverPostalPlace || "")),
    apply: { ShipFromName: "SEEDCO", ShipAcountNumber: "REASSIGN_SEEDCO" },
  },
];
