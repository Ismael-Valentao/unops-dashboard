...
async function aggregateFornecido(rows, options = {}) {
  ...
  for (const r of rows || []) {
    ...
    if (!byKey.has(key)) {
      byKey.set(key, {
        ...
        // Subdivisão de LEVANTADO em "já na app UNOPS" vs "pendente lançar"
        app_kg: 0, app_qty_un: 0, count_app: 0,
        pend_app_kg: 0, pend_app_qty_un: 0, count_pend_app: 0,
        by_sku: {},
        by_province: {},
        origins: new Set(),
      });
    ...
    if (isForn) {
      ...
      // Subdividir LEVANTADO entre app vs pend-app
      if (gtuInApp) {
        x.count_app++;
        x.app_kg += w;
    ...
    if (!x.by_sku[sku]) {
      x.by_sku[sku] = {
        ...
        pend_app_weight_kg: 0, pend_app_volumes: 0, pend_app_count: 0,
      };
    }
    ...
    if (isForn) {
      ...
      if (gtuInApp) {
        ...
        skuEntry.pend_app_weight_kg += w;
        skuEntry.pend_app_volumes   += v;
        skuEntry.pend_app_count++;
      }
    } else {
  ...
  function makeSkuItem(sku, e) {
    ...
    const pend_app_qty = e.is_saco ? e.pend_app_volumes : Math.round(e.pend_app_weight_kg);
    ...
    return {
      ...
      app_qty,                 // já está na app UNOPS (GTU bate)
      pend_app_qty,            // saiu (LEVANTADO) mas falta lançar na app
      forn_count: e.forn_count,
      pend_count: e.pend_count,
      app_count: e.app_count,
      pend_app_count: e.pend_app_count,
      pct_recebido: pct,
  ...
  for (const [key, x] of byKey) {
    ...
    for (const s of x.by_sku) {
      ...
      pe.pend_app_qty       += s.pend_app_qty;
      pe.skus.push(s.sku);
    }
    ...
    for (const [p, meta] of Object.entries(metas)) {
      ...
      products.push({
        ...
        app_qty:      pe ? pe.app_qty      : 0,
        pend_app_qty: pe ? pe.pend_app_qty : 0,
  ...
  for (const x of suppliers) {
    ...
    for (const p of x.by_product) {
      ...
      tp.pend_app_qty += (p.pend_app_qty || 0);
  ...
  for (const [metaKey, productsMeta] of Object.entries(METAS)) {
    ...
    suppliers.push({
      ...
      count_app: 0, count_pend_app: 0,
      forn_kg: 0, forn_qty_un: 0,
      pend_kg: 0, pend_qty_un: 0,
      app_kg: 0, app_qty_un: 0,
      pend_app_kg: 0, pend_app_qty_un: 0,
      by_sku: [],
      by_product,
      by_province: [],
      origins: [],
  ...
  for (const x of suppliers) {
    for (const p of x.by_product) {
      ...
      cur.pend_app_qty += (p.pend_app_qty || 0);
      if (p.meta_qty != null) {
  ...
  const totals = {
    ...
    services_pend_app: suppliers.reduce((s, x) => s + (x.count_pend_app || 0), 0),
    forn_kg:     suppliers.reduce((s, x) => s + x.forn_kg,     0),
    forn_qty_un: suppliers.reduce((s, x) => s + x.forn_qty_un, 0),
    // Subdivisão de LEVANTADO em APP vs PEND APP
    app_kg:      suppliers.reduce((s, x) => s + (x.app_kg || 0), 0),
    app_qty_un:  suppliers.reduce((s, x) => s + (x.app_qty_un || 0), 0),
    pend_app_kg: suppliers.reduce((s, x) => s + (x.pend_app_kg || 0), 0),
    pend_app_qty_un: suppliers.reduce((s, x) => s + (x.pend_app_qty_un || 0), 0),
