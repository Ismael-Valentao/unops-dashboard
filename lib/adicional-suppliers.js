...
function aggregateFornecido(rows) {
  ...
  for (const [key, x] of byKey) {
    ...
    const products = [];
    let meta_total_kg = 0, forn_total_for_meta_kg = 0, has_meta_products = 0;
    // Adiciona produtos com meta (mesmo que ainda não tenham forn)
    const seenProducts = new Set();
    for (const [p, meta] of Object.entries(metas)) {
      const pe = productMap.get(p);
      const forn_kg = pe ? pe.forn_weight_kg : 0;
      const forn_un = pe ? pe.forn_volumes  : 0;
      const is_saco = pe ? pe.is_saco       : false;
      ...
      const meta_pct      = meta_qty == null || meta_qty === 0
        ? null
        : Math.round((forn_qty / meta_qty) * 100);
      products.push({
        product: p,
      ...
      seenProducts.add(p);
      if (meta_qty != null) {
        meta_total_kg += is_saco ? 0 : meta_qty; // só conta kg, não mistura un/kg
        forn_total_for_meta_kg += is_saco ? 0 : forn_qty;
        has_meta_products++;
      }
    }
    ...
    x.meta_pct = meta_total_kg > 0
      ? Math.min(999, Math.round((forn_total_for_meta_kg / meta_total_kg) * 100))
      : null;
  ...
  for (const x of suppliers) {
    ...
    // Recalcula totals do dst (meta_total_kg fica inalterada — mesma meta_key)
    let fornForMeta = 0;
    for (const p of dst.by_product) {
      if (p.has_meta && !p.is_saco) fornForMeta += p.forn_qty;
    }
    dst.meta_pendente_total_kg = Math.max(0, dst.meta_total_kg - fornForMeta);
    dst.meta_pct = dst.meta_total_kg > 0
      ? Math.min(999, Math.round((fornForMeta / dst.meta_total_kg) * 100)) : null;
  }
  // Substitui suppliers pela lista pós-merge
  suppliers.length = 0;
  suppliers.push(...finalSuppliers);

  // Ordena: fornecedores com meta primeiro (por meta kg desc), depois sem meta
  suppliers.sort((a, b) => {
    if (a.has_meta !== b.has_meta) return a.has_meta ? -1 : 1;
    if (a.has_meta) return b.meta_total_kg - a.meta_total_kg;
    return b.forn_kg - a.forn_kg;
  });

  // Agregados globais por PRODUTO canónico (somando todos os fornecedores).
  // Substitui o antigo by_sku — operação prefere ver por produto, não por SKU.
  ...
  const totals = {
    ...
    // Metas (em kg — sacos não incluídos em meta_kg)
    ...
    meta_pct_global:    null, // calculado a seguir
    by_product,
  };
