...
// Combinações (fornecedor + SKU) a esconder — para casos em que o fornecedor
// aparece com um produto que não devia fornecer (registo erróneo na API).
// Aplicado a row-level antes da agregação, depois das reassignments.
// Cada entry: { supplier: RegExp, skus: Array<string>, note?: string }
//
// Nota: actualmente vazio. PHOENIX+Feijão foi movido para reassignment
// (supplier-reassignments.js) — agora os 7.500 kg vão para SEEDCO em vez
// de serem perdidos. Manter este mecanismo disponível para casos futuros
// onde a operação queira realmente DESCARTAR rows (não mover).
const HIDDEN_ROW_COMBOS = [];

function _matchesHiddenCombo(row) {
  const name = String(row.ShipFromName || "");
  const sku  = String(row.SKU || "");
  for (const combo of HIDDEN_ROW_COMBOS) {
    if (combo.supplier.test(name) && combo.skus.includes(sku)) return true;
  }
  return false;
}
...
/**
 * @param {Array} rows                — Rows da API ADICIONAL
 * @param {Object} [options]
 * @param {Set<string>} [options.unopsGtus] — GTUs normalizados que JÁ estão na
 *   sheet UNOPS. Quando passado, separa LEVANTADO em duas categorias:
 *     - APP            = LEVANTADO ∩ UNOPS  (batedor já lançou na app UNOPS)
 *     - PEND APP       = LEVANTADO − UNOPS  (camião saiu mas batedor ainda
 *                                            não lançou na app)
 *   Sem este parâmetro, app_qty fica = forn_qty e pend_app_qty = 0.
 */
async function aggregateFornecido(rows, options = {}) {
  ...
  // Pré-carrega cache de PRODUCT METAS (globais por produto)
  const { primeProductMetasCache, getProductMeta, listAllProductMetas } = require("./product-metas");
  ...
  // Filtra combinações fornecedor+SKU mal atribuídas (ex: PHOENIX + Feijão)
  rows = rows.filter((r) => !_matchesHiddenCombo(r));

  // Set de GTUs da sheet UNOPS para detectar quais LEVANTADOS já foram
  // lançados na app. Vem normalizado pelo caller (app.js).
  const unopsGtus = options.unopsGtus || null;
  const { normGtu } = require("./adicional-match");
  ...
  for (const r of rows || []) {
    ...
    if (!byKey.has(key)) {
      byKey.set(key, {
        ...
        // Subconjunto de LEVANTADO cujo GTU JÁ está na sheet UNOPS (lançado na app).
        // PEND APP é derivado da meta: max(0, meta − app), não desta acumulação.
        app_kg: 0, app_qty_un: 0, count_app: 0,
        by_sku: {},
        by_province: {},
        origins: new Set(),
      });
    ...
    // Saber se o GTU desta guia já foi lançado na app UNOPS.
    // - Se não temos unopsGtus (caller não passou) → trata tudo como "na app"
    //   (mantém compat com chamadas legacy onde app_qty = forn_qty)
    // - Se temos: usa normGtu para fazer match robusto (cobre variantes
    //   "GTU/98/..." vs "GTU98/...", padding, typos GTUS→GTS)
    const gtuInApp = unopsGtus
      ? unopsGtus.has(normGtu(r.ClientBarCode))
      : true;
    ...
    if (isForn) {
      ...
      // Acumula só o LEVANTADO que JÁ está na app UNOPS. PEND APP vem
      // depois como max(0, meta − app) a nível de produto/totais.
      if (gtuInApp) {
        x.count_app++;
        x.app_kg += w;
        if (saco) x.app_qty_un += v;
      }
    } else {
      x.count_pend++;
      x.pend_kg += w;
    ...
    if (!x.by_sku[sku]) {
      x.by_sku[sku] = {
        ...
        app_weight_kg: 0, app_volumes: 0, app_count: 0,
      };
    }
    ...
    if (isForn) {
      ...
      skuEntry.forn_count++;
      if (gtuInApp) {
        skuEntry.app_weight_kg += w;
        skuEntry.app_volumes   += v;
        skuEntry.app_count++;
      }
    } else {
  ...
  function makeSkuItem(sku, e) {
    ...
    const app_qty  = e.is_saco ? e.app_volumes  : Math.round(e.app_weight_kg);
    ...
    return {
      ...
      app_qty,                 // subset de forn_qty cujo GTU está na sheet UNOPS
  ...
  for (const [key, x] of byKey) {
    ...
    x.app_kg      = Math.round(x.app_kg);
    x.pend_app_kg = Math.round(x.pend_app_kg);
    ...
    const products = [];
    let meta_total_kg = 0, forn_total_for_meta_kg = 0, app_total_for_meta_kg = 0;
    let meta_total_un = 0, forn_total_for_meta_un = 0, app_total_for_meta_un = 0;
    ...
    for (const [p, meta] of Object.entries(metas)) {
      ...
      const app_qty  = pe ? pe.app_qty : 0;
      const meta_qty = meta == null ? null : Number(meta);
      const meta_pendente = meta_qty == null ? null : Math.max(0, meta_qty - forn_qty);
      // PEND APP = diferença entre META e o que está na app UNOPS
      // (não LEVANTADO − APP, mas META − APP — gap total face ao contratado)
      const pend_app_qty  = meta_qty == null ? null : Math.max(0, meta_qty - app_qty);
      ...
      products.push({
        ...
        app_qty,                 // LEVANTADO ∩ sheet UNOPS
        pend_app_qty,            // META − APP (gap face ao contratado)
      ...
      if (meta_qty != null) {
        if (is_saco) {
          ...
          app_total_for_meta_un += app_qty;
        } else {
          meta_total_kg += meta_qty;
          forn_total_for_meta_kg += forn_qty;
          app_total_for_meta_kg += app_qty;
        }
        has_meta_products++;
      }
    }
    ...
    for (const [p, pe] of productMap) {
      ...
      products.push({
        ...
        app_qty: pe.app_qty,
        pend_app_qty: null,   // sem meta → não há PEND APP definido
        meta_qty: null,
        meta_pendente: null,
        meta_pct: null,
        has_meta: false,
    ...
    // PEND APP totals: META − APP (sempre ≥ meta_pendente)
    x.meta_pendente_app_total_kg = Math.max(0, meta_total_kg - app_total_for_meta_kg);
    x.meta_pendente_app_total_un = Math.max(0, meta_total_un - app_total_for_meta_un);
  ...
  for (const x of suppliers) {
    ...
    for (const p of x.by_product) {
      ...
      tp.app_qty      += (p.app_qty || 0);
      tp.is_saco      = tp.is_saco || p.is_saco;
      tp.skus         = [...new Set([...tp.skus, ...p.skus])];
      // Recalcula meta_pendente, pend_app_qty e pct
      if (tp.meta_qty != null) {
        tp.meta_pendente = Math.max(0, tp.meta_qty - tp.forn_qty);
        tp.pend_app_qty  = Math.max(0, tp.meta_qty - tp.app_qty);
    ...
    let fornForMetaKg = 0, fornForMetaUn = 0, appForMetaKg = 0, appForMetaUn = 0;
    for (const p of dst.by_product) {
      if (!p.has_meta) continue;
      if (p.is_saco) { fornForMetaUn += p.forn_qty; appForMetaUn += (p.app_qty || 0); }
      else           { fornForMetaKg += p.forn_qty; appForMetaKg += (p.app_qty || 0); }
    }
    ...
    dst.meta_pendente_app_total_kg = Math.max(0, dst.meta_total_kg - appForMetaKg);
    dst.meta_pendente_app_total_un = Math.max(0, (dst.meta_total_un || 0) - appForMetaUn);
  ...
  for (const [metaKey, productsMeta] of Object.entries(METAS)) {
    ...
    const by_product = validMetas.map(([prod, meta]) => {
      ...
      return {
        ...
        app_qty: 0,
        pend_app_qty: meta_qty,  // META − 0 = META
        meta_qty,
    ...
    suppliers.push({
      ...
      meta_pendente_app_total_kg: meta_total_kg,  // tudo ainda em PEND APP
      meta_pendente_app_total_un: meta_total_un,
      meta_pct: pct,
  ...
  try {
    ...
    // Passa rows (já reassignadas) para que ajustes do tipo
    // "placeholder_minus_matched" possam contar rows reais com sentinel.
    applyAdjustments(suppliers, rows);
  } catch (e) {
    ...
    for (const p of x.by_product) {
      ...
      if (p.has_meta || p.forn_qty > 0) {
        cur.suppliers.push({
          ...
          forn_qty: p.forn_qty,
          app_qty: p.app_qty || 0,
          pend_app_qty: p.pend_app_qty,  // já = max(0, meta − app) ou null
          meta_qty: p.meta_qty,
  ...
  // SOBREPOSIÇÃO POR PRODUCT META GLOBAL:
  // Para cada produto, se houver entry em product_metas, usa essa qty como
  // meta GLOBAL do produto (no topo do card) em vez da soma das supplier_metas.
  // As supplier_metas continuam a aparecer dentro do card por fornecedor.
  for (const cur of prodTotals.values()) {
    const pm = getProductMeta(cur.product);
    if (pm && pm.qty != null) {
      cur.meta_qty       = Number(pm.qty);
      cur.meta_pendente  = Math.max(0, cur.meta_qty - cur.forn_qty);
      cur.has_meta_any   = true;
      cur._meta_source   = "product";  // tag p/ debug
      cur._meta_match    = pm.matched_key;
      // Se a meta de produto é em "un" mas o is_saco do produto não bate,
      // confiar na product_meta (operação sabe melhor).
      if (pm.unit === "un") cur.display_unit = "un";
    } else {
      cur._meta_source = "sum-of-suppliers";
    }
    // PEND APP global = max(0, meta GLOBAL − app_qty agregado)
    cur.pend_app_qty = cur.has_meta_any
      ? Math.max(0, (cur.meta_qty || 0) - (cur.app_qty || 0))
      : null;
  }

  // Adiciona produtos que existem em product_metas mas NÃO foram entregues
  // por nenhum fornecedor — aparecem com 0% no topo da página.
  const allProductMetas = listAllProductMetas();
  for (const [productName, info] of Object.entries(allProductMetas)) {
    // Canonical match: já está em prodTotals?
    const exists = [...prodTotals.values()].some((p) => {
      const a = String(p.product).toLowerCase().trim();
      const b = String(productName).toLowerCase().trim();
      return a === b || a.includes(b) || b.includes(a);
    });
    if (exists) continue;
    if (info.qty == null) continue;
    const isSaco = info.unit === "un";
    prodTotals.set(productName, {
      product: productName,
      is_saco: isSaco,
      display_unit: info.unit || "kg",
      forn_qty: 0,
      criado_qty: 0,
      app_qty: 0, pend_app_qty: 0,
      meta_qty: Number(info.qty),
      meta_pendente: Number(info.qty),
      has_meta_any: true,
      suppliers: [],
      _meta_source: "product-only",
      _meta_match: productName,
    });
  }
  ...
  const totals = {
    ...
    services_app: suppliers.reduce((s, x) => s + (x.count_app || 0), 0),
    forn_kg:     suppliers.reduce((s, x) => s + x.forn_kg,     0),
    forn_qty_un: suppliers.reduce((s, x) => s + x.forn_qty_un, 0),
    // APP = subset de LEVANTADO com GTU já lançado na app UNOPS
    app_kg:      suppliers.reduce((s, x) => s + (x.app_kg || 0), 0),
    app_qty_un:  suppliers.reduce((s, x) => s + (x.app_qty_un || 0), 0),
    ...
    // PEND APP = META − APP (agregado dos suppliers c/ meta)
    meta_pendente_app_kg: suppliers.reduce((s, x) => s + (x.meta_pendente_app_total_kg || 0), 0),
    meta_pendente_app_un: suppliers.reduce((s, x) => s + (x.meta_pendente_app_total_un || 0), 0),
    meta_pct_global:    null,
    meta_pct_sacos:     null,
    unops_matched_gtus: unopsGtus ? unopsGtus.size : null,  // info p/ debug
    by_product,
  };
