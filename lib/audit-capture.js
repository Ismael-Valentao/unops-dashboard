...
async function captureRows(rows) {
  if (!Array.isArray(rows) || !rows.length) {
    return { inserted: 0, updated_status: 0, seen: 0, total: 0, errors: [] };
  }

  const stats = { inserted: 0, updated_status: 0, seen: 0, total: rows.length, errors: [] };
    ...
    for (const r of batch) {
      try {
        ...
        const [existing] = await getPool().query(
          "SELECT id, verification_status FROM delivery_audit WHERE dedup_key = ?",
          [dedupKey]
        );
          ...
          if (status && existingRow.verification_status !== status) {
            ...
            await query(
              `UPDATE delivery_audit
               SET verification_status = ?, status_changed_at = NOW(), last_seen_at = NOW()
               WHERE id = ?`,
            ...
            // Sem mudança → só toca last_seen_at
            await query(
              "UPDATE delivery_audit SET last_seen_at = NOW() WHERE id = ?",
              [existingRow.id]
            );
            stats.seen++;
          }
          ...
          await query(
            `INSERT INTO delivery_audit
             (dedup_key, gtu, adsn, beneficiary_name, extensionist_id, nuit,
              ...
              submitted_by, delivery_date_iso, verification_status, raw_data)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
              dedupKey, gtu, adsn, benefName, benefId, null,
              product, qty, packages, unit, district, province,
              submitter, dDateIso, status, rawData,
            ]
          );
          stats.inserted++;
        }
